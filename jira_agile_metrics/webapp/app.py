...
def get_real_results():
    ...
    if not options['connection'].get('username') or not options['connection'].get('password') or not options['connection'].get('domain'):
        raise RuntimeError("JIRA credentials (domain, username, password) must be set in config.yml under the Connection section.")
...
def cfd_chart():
    try:
        ...
        if chart_data is None or len(chart_data.index) == 0:
            ...
            return render_template('bokeh_chart.html', title="Cumulative Flow Diagram (CFD)", script="", div="")
        p = figure(title="Cumulative Flow Diagram (CFD)", x_axis_type='datetime', width=800, height=400)
        for col in chart_data.columns:
            p.line(chart_data.index, chart_data[col], legend_label=col, line_width=2)
        p.legend.location = "top_left"
        p.xaxis.axis_label = "Date"
        p.yaxis.axis_label = "Items"
        script, div = components(p)
        return render_template('bokeh_chart.html', title="Cumulative Flow Diagram (CFD)", script=script, div=div)
    except Exception as e:
        flash(str(e), "danger")
        return render_template('bokeh_chart.html', title="Cumulative Flow Diagram (CFD)", script="", div="")
...
def histogram_chart():
    try:
        ...
        p = figure(title="Cycle Time Histogram", x_range=list(chart_data.index), width=800, height=400)
        p.vbar(x=list(chart_data.index), top=list(chart_data.values), width=0.9)
        p.xaxis.axis_label = "Cycle Time Bin"
        p.yaxis.axis_label = "Items"
        script, div = components(p)
        return render_template('bokeh_chart.html', title="Cycle Time Histogram", script=script, div=div)
    except Exception as e:
        flash(str(e), "danger")
        return render_template('bokeh_chart.html', title="Cycle Time Histogram", script="", div="")
...
def scatterplot_chart():
    try:
        ...
        if chart_data is None or len(chart_data.index) == 0:
            ...
            return render_template('bokeh_chart.html', title="Cycle Time Scatterplot", script="", div="")
        ...
        script, div = components(p)
        return render_template('bokeh_chart.html', title="Cycle Time Scatterplot", script=script, div=div)
    except Exception as e:
        flash(str(e), "danger")
        return render_template('bokeh_chart.html', title="Cycle Time Scatterplot", script="", div="")
...
def ageingwip_chart():
    try:
        ...
        p = figure(title="Ageing WIP Chart", x_range=list(chart_data['status'].astype(str)), width=800, height=400)
...
def debt_chart():
    try:
        ...
        p.line(chart_data['created'], chart_data['age'].dt.days, legend_label="Debt Age", line_width=2)
        p.xaxis.axis_label = "Created Date"
        p.yaxis.axis_label = "Age (days)"
        script, div = components(p)
        return render_template('bokeh_chart.html', title="Technical Debt Chart", script=script, div=div)
    except Exception as e:
        flash(str(e), "danger")
        return render_template('bokeh_chart.html', title="Technical Debt Chart", script="", div="")
...
def debt_age_chart():
    try:
        ...
        p = figure(title="Debt Age Chart", x_range=list(chart_data['priority'].astype(str)), width=800, height=400)
        p.vbar(x=list(chart_data['priority'].astype(str)), top=list(chart_data['age'].dt.days), width=0.9)
...
def defects_priority_chart():
    try:
        ...
        p = figure(title="Defects by Priority", x_range=list(chart_data['priority'].astype(str)), width=800, height=400)
        p.vbar(x=list(chart_data['priority'].astype(str)), top=list(chart_data['key'].value_counts()), width=0.9)
        p.xaxis.axis_label = "Priority"
        p.yaxis.axis_label = "Count"
        script, div = components(p)
        return render_template('bokeh_chart.html', title="Defects by Priority", script=script, div=div)
    except Exception as e:
        flash(str(e), "danger")
        return render_template('bokeh_chart.html', title="Defects by Priority", script="", div="")
...
def defects_type_chart():
    try:
        ...
        p = figure(title="Defects by Type", x_range=list(chart_data['type'].astype(str)), width=800, height=400)
        p.vbar(x=list(chart_data['type'].astype(str)), top=list(chart_data['key'].value_counts()), width=0.9)
...
def defects_environment_chart():
    try:
        ...
        if chart_data is None or len(chart_data.index) == 0:
            ...
            return render_template('bokeh_chart.html', title="Defects by Environment", script="", div="")
        p = figure(title="Defects by Environment", x_range=list(chart_data['environment'].astype(str)), width=800, height=400)
        p.vbar(x=list(chart_data['environment'].astype(str)), top=list(chart_data['key'].value_counts()), width=0.9)
        p.xaxis.axis_label = "Environment"
        p.yaxis.axis_label = "Count"
        script, div = components(p)
        return render_template('bokeh_chart.html', title="Defects by Environment", script=script, div=div)
    except Exception as e:
        flash(str(e), "danger")
        return render_template('bokeh_chart.html', title="Defects by Environment", script="", div="")
...
def impediments_chart():
    try:
        ...
        p = figure(title="Impediments Chart", x_range=list(chart_data['status'].astype(str)), width=800, height=400)
        p.vbar(x=list(chart_data['status'].astype(str)), top=list(chart_data['key'].value_counts()), width=0.9)
        p.xaxis.axis_label = "Status"
        p.yaxis.axis_label = "Count"
        script, div = components(p)
        return render_template('bokeh_chart.html', title="Impediments Chart", script=script, div=div)
    except Exception as e:
        flash(str(e), "danger")
        return render_template('bokeh_chart.html', title="Impediments Chart", script="", div="")
...
def waste_chart():
    try:
        ...
        p = figure(title="Waste Chart", x_range=list(chart_data['last_status'].astype(str)), width=800, height=400)
        p.vbar(x=list(chart_data['last_status'].astype(str)), top=list(chart_data['key'].value_counts()), width=0.9)
...
def progress_chart():
    try:
        ...
        p.yaxis.axis_label = "Y"
        script, div = components(p)
        return render_template('bokeh_chart.html', title="Progress Report Chart", script=script, div=div)
    except Exception as e:
        flash(str(e), "danger")
        return render_template('bokeh_chart.html', title="Progress Report Chart", script="", div="")
...
def percentiles_chart():
    try:
        ...
        p = figure(title="Percentiles Chart", x_range=[str(q) for q in chart_data.index], width=800, height=400)
        ...
        script, div = components(p)
        return render_template('bokeh_chart.html', title="Percentiles Chart", script=script, div=div)
    except Exception as e:
        flash(str(e), "danger")
        return render_template('bokeh_chart.html', title="Percentiles Chart", script="", div="")
...
def cycletime_chart():
    try:
        ...
        p = figure(title="Cycle Time Chart", x_range=list(chart_data['key'].astype(str)), width=800, height=400)
        p.vbar(x=list(chart_data['key'].astype(str)), top=list(chart_data['cycle_time'].dt.days), width=0.9)
