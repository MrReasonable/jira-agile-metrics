...
```bash
docker run --rm -v /path/to/config:/config -v /path/to/output:/data
optilude/jira-agile-metrics:batch-latest
```
...
```bash
docker run -d --rm -p 8080:80 --name jira_metrics
optilude/jira-agile-metrics:server-latest
```
...
jira-agile-metrics --username your-email@example.com --password CGI...
config.yml
```
...
```sh
# Connects with an API token that is passed on --password argument
jira-agile-metrics --username your-email@example.com --key 6d1658b... --token
a7d6b34... config.yml
```
...
`Backlog`, `Committed`, `Elaboration`, `Build`, `Code review`, `Test`,
and `Done`,
and allowing a set of resolutions like `Completed`, `Withdrawn`,
and `Duplicate`.
...
Here is an example JIRA configuration file for a basic example using the
workflow
above:
...
Query: Project=ABC AND IssueType=Story AND (Resolution IS NULL OR Resolution IN
(Completed, Withdrawn))
...
Output:
    ...
    # CSV files with raw data for input to other tools
    or further analysis in a spreadsheet
    # If you use .json or .xlsx as the extension, you can get JSON data files
    or Excel
    ...
    Burnup forecast chart trials: 100 # number of Monte Carlo trials to run
to estimate completion date

    # Burnup forecast chart throughput window: 60 # Days in the past
to use for calculating historical throughput
    # Burnup forecast chart throughput window end: 2018-06-01 # Calculate
throughput window to this date (defaults to today)
    # Burnup forecast chart target: 100 # items to complete in total; by
default uses the current size of the backlog
    # Burnup forecast chart deadline: 2018-06-01 # deadline date, in ISO
format; if not set, no deadline is drawn.
    # Burnup forecast chart deadline confidence: .85 # percentile to use to
compare forecast to deadline
...
**Note:** In the configuration file, you can specify output file _names_, but
...
![Cumulative Flow Diagram](./docs/images/cfd.png)
...
![Cycle time scatter plot](./docs/images/scatterplot.png)
...
![Cycle time histogram](./docs/images/histogram.png)
...
![Throughput trend](./docs/images/throughput.png)
...
![Work in Progress](./docs/images/wip.png)
...
![Net flow](./docs/images/net-flow.png)
...
![Ageing WIP](./docs/images/ageing-wip.png)
...
![Burn-up chart](./docs/images/burnup.png)
...
![Burn-up forecast](./docs/images/burnup-forecast.png)
...
to estimate completion date
    ...
    Burnup forecast chart throughput window: 60 # Days in the past to use
for calculating historical throughput
    Burnup forecast chart throughput window end: 2018-06-01 # Calculate
throughput window to this date (defaults to last day of burnup)
    Burnup forecast chart target: 100 # items to complete in total; by default
uses the current size of the backlog
    Burnup forecast chart deadline: 2018-06-01 # deadline date, in ISO format;
if not set, no deadline is drawn.
    Burnup forecast chart deadline confidence: .85 # percentile to use to
compare forecast to deadline
```
...
_Note this feature is not available in Trello._
...
![Number of impediments](./docs/images/impediments.png)

![Total impeded days](./docs/images/impediments-days.png)

![Number of impediments by status](./docs/images/impediments-status.png)

![Total impeded days by status](./docs/images/impediments-status-days.png)
...
![Defects by priority](./docs/images/defects-by-priority.png)

![Defects by type](./docs/images/defects-by-type.png)

![Defects by environment](./docs/images/defects-by-environment.png)
...
![Trello Agile Actionable](./docs/images/trello-agile-actionable.png)
![Trello source](./docs/images/trello-source.png)

## Technical debt

Two charts that show the nature and age of recorded (unresolved) technical debt.

![Technical debt](./docs/images/tech-debt.png)

![Technical debt age](./docs/images/tech-debt-age.png)
...
libraries (the _Bootstrap_ CSS/JS library and its _jQuery_ dependency; the
_Fontawesome_ icon font), but can be served from any static web server or opened
...
![Progress report](./docs/images/progressreport.png)

The status report is based on the principle of _outcomes_ (e.g. projects,
releases, objectives) that are broken down into _epics_, which will eventually
be further broken down into _stories_. Epics are assumed to be fully owned by
exactly one _team_. Epics and stories are represented by JIRA tickets
...
Output:
    ...
    Progress report epic query template: project = ABC AND type = Epic AND
Outcome = {outcome} ORDER BY created

    # The query used to identify stories for an epic. The placeholders
    # `{epic}`, `{outcome}`, and `{team}` may be used to parameterise the
    # query.
    Progress report story query template: project = ABC AND type = Story AND
"Epic link" = {epic}
...
```yaml
    Progress report outcome query: project = ABC AND type = Outcome AND
resolution IS EMPTY ORDER BY summary
...
Output:
    ...
    Progress report epic query template: project = ABC AND type = Epic AND
resolution IS EMPTY ORDER BY created
    Progress report story query template: project = ABC AND type = Story AND
"Epic link" = {epic}
    Progress report teams:
        - Name: Default
...
When specifying attributes, use the _name_ of the field (as rendered on screen
...
- `Bottleneck max issues: <number>` – Maximum number of issues to include in the
  per-issue bottleneck stacked chart. Must be a positive integer. Defaults
  to 30.
...
- `Throughput window: <number>` – Number of recent periods to show in
  throughput chart. Defaults to showing all periods.
...
- `Progress report outcome query: <query>` – Allows outcomes to be enumerated
  ...
  If used, `Progress report epic query template` _must_ be set.
- `Progress report epic deadline field: <fieldname>` – Name of a date field
...
- Make sure change history is always iterated in date order, even if JIRA
  returns
  them differently.
